...
def simulate_replies(db: Session, c: Campaign, count: int, intent: str | None = None) -> dict:
    """Fires up to `count` simulated replies, each a random prospect, using `intent` if given or
    a random one per reply otherwise. Stops early (without erroring) once no more prospects are
    eligible, since running out of prospects to reply for isn't a failure, just the end of what's
    simulate-able."""
    sent = []
    for _ in range(count):
        try:
            sent.append(simulate_reply(db, c, intent=intent))
